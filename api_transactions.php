...
if ($method === 'POST' && $action === 'create') {
    ...
    // Validate stock before creating the transaction so stock-tracked items
    // like LPG tanks and accessories cannot be oversold.
    foreach ($items as $item) {
        if (($item['item_type'] ?? '') !== 'product' || empty($item['product_id'])) continue;

        $pid = $item['product_id'];
        $qty = (float)($item['qty'] ?? 0);
        if ($qty <= 0) jsonError('Invalid product quantity');

        $prodRes = $sb->select('products', "id=eq.{$pid}&select=id,name,stock_qty,is_active");
        if (empty($prodRes['data'])) jsonError('Product not found', 404);

        $product = $prodRes['data'][0];
        if (empty($product['is_active'])) jsonError(($product['name'] ?? 'Product') . ' is inactive');
        if ((float)$product['stock_qty'] < $qty) {
            jsonError('Insufficient stock for ' . ($product['name'] ?? 'product'));
        }
    }
