...
$pageTitle = 'Oil Receiving';
$activePage = 'oil_receiving.php';
$allowedCategories = ['Engine Oil', 'Gear Oil', 'ATF/Fluids'];
include __DIR__ . '/layout.php';
?>
<div style="display:flex;flex-direction:column;gap:20px">
  <h1 style="margin:0;font-size:22px;font-weight:800">Oil Inventory Receiving</h1>
  <div class="card">
    <div class="card-header"><span>📥</span><h3 class="card-title">New Oil Stock Receipt</h3></div>
    <div class="card-body" style="display:flex;flex-direction:column;gap:16px">
      ...
      <div>
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px"><span style="font-weight:700;font-size:14px">Oil Items</span><button class="btn btn-sm btn-primary" onclick="addReceivingRow()">+ Add Item</button></div>
        <div class="table-wrap"><table class="table" style="min-width:600px"><thead><tr><th>Product</th><th>Qty</th><th>Unit Cost</th><th>Line Total</th><th></th></tr></thead><tbody id="rcv-items-tbody"><tr id="rcv-empty-row"><td colspan="5" style="text-align:center;color:var(--text-muted);padding:20px">No oil items added yet.</td></tr></tbody><tfoot><tr style="font-weight:700;background:var(--surface2)"><td colspan="3" style="padding:10px 14px;text-align:right">Total:</td><td style="padding:10px 14px" id="rcv-total" class="text-mono">₱0.00</td><td></td></tr></tfoot></table></div>
      </div>
      <div style="display:flex;justify-content:flex-end"><button class="btn btn-success btn-lg" id="save-receipt-btn" onclick="saveReceipt()">Save Oil Receipt & Update Stock</button></div>
    </div>
  </div>
</div>
...
async function saveReceipt() {
  ...
  if (!rows.length) { App.toast('Add at least one oil item', 'error'); return; }
  const items = [];
  let valid = true;
  ...
  if (!valid) { App.toast('Please fill all oil item fields correctly', 'error'); return; }
  const confirmed = await App.confirm(`Receive <strong>${items.length}</strong> oil item(s) and update stock?<br><small style="color:var(--text-muted)">Total: ${document.getElementById('rcv-total').textContent}</small>`, 'Confirm Oil Receiving', { icon: 'question', confirmText: 'Yes, receive & update stock', confirmColor: '#16a34a' });
  if (!confirmed) return;
  ...
  try {
    ...
    App.toast('Oil receipt saved & stock updated!', 'success');
    location.reload();
  } catch (err) { App.toast(err.message, 'error'); App.loading.hide(btn); }
}
async function removeReceivingRow(id) { if (await App.confirm('Remove this oil item from the list?', 'Remove Item', true)) { document.getElementById(id)?.remove(); updateGrandTotal(); } }
